# CLAUDE_DETAIL.md — Energy Contracts 상세 (catalog / 아키텍처 / 이력)

> **본 문서 = `CLAUDE.md` 에서 분리한 상세 catalog·아키텍처·버전 이력.** CLAUDE.md 는 identity + mirror 헤더(drift-gate) + SSOT 변경 절차 + 실행/검증 명령만 유지한다. 이 문서는 verbatim 이관본(2026-07-04 diet).
